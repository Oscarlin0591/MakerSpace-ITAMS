...
export async function deleteEmail(email: String) {
  const { error } = await supabase.from('email_recipient').delete().eq('email', email);
...
export async function getEmail(email: string | void) {
  if (typeof email === 'string') {
    ...
    const newEmail: EmailRecipient = new EmailRecipient(
      data.data?.email,
      data.data?.alert_notifications,
      data.data?.daily_notifications,
      data.data?.weekly_notifications,
    );
    data.data = newEmail;
    return data;
  } else {
    ...
    data.data?.forEach((email) => {
      let newEmail = new EmailRecipient(
        email.email,
        email.alert_notifications,
        email.daily_notifications,
        email.weekly_notifications,
      );
